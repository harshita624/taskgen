...
export default function TaskForm({ onTaskCreated }: { onTaskCreated?: () => void }) {
  ...
  const handlePriorityChange = (newPriority: "high" | "medium" | "low") => {
    ...
    // prevent focus sticking on button
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              {title && <div className="absolute right-3 top-1/2 transform -translate-y-1/2"><div className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></div></div>}
            </div>
            <p className="text-xs text-gray-500">Be clear and specific about the task</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <textarea
                ...
                maxLength={maxWords}
              />
              <div className="absolute bottom-3 right-3 text-xs text-gray-400">{wordCount}/{maxWords}</div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                {(["high", "medium", "low"] as const).map((p) => (
                  <button
                    ...
                    disabled={loading}
                    className={`w-full flex items-center gap-3 p-3 border-2 rounded-xl transition-all duration-200 hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed ${getPriorityColor(
                      p,
                      priority === p
                    )}`}
                  >
                    ...
                    {priority === p && <span className="ml-auto w-2 h-2 bg-current rounded-full"></span>}
                  </button>
                ))}
              </div>
              <p className="text-xs text-gray-500">Current: {priority}</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              {categories.length === 0 && <p className="text-xs text-amber-600">Loading categories...</p>}
            </div>
          </div>
          ...
          <div className="pt-4">
            ...
            <p className="text-center text-xs text-gray-500 mt-3">
              Click to create task. Use priority buttons above.
            </p>
          </div>
        </div>
      </div>
